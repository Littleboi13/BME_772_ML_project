...
function [meanEEG, stdEEG, kurtEEG, skewEEG, AR_coeff] = time_domain_features(signal)
    ...
    for ch = 1:numCh
        ...
        kurtEEG(ch) = kurtosis(x);
        skewEEG(ch) = skewness(x);
...
function [PSD, PSD_norm, FT, FT_norm, totalPower] = feature_domain_spectral_features(signal, Fs)
    ...
    % specEntropyArr = zeros(1, numCh); % preallocate
...
for i = 1:numel(files)
    ...
    if ~isfield(data, 'bp_eeg')
        warning("File %s has no transposed 'eeg' field. Skipping.\n", files(i).name);
        continue;
    end
    ...
    [PSD, FT] = feature_domain_spectral_features(eeg, Fs);
    ...
    size(meanEEG), size(stdEEG), size(kurtEEG), size(skewEEG), size(AR_str)
    ...
    features = table(meanEEG, stdEEG, kurtEEG, skewEEG, AR_str, ...
                     ...
                     'VariableNames', {'MeanEEG', 'StdEEG', 'Kurtosis', 'Skewness', 'AR_Coefficients', ...
                                       ...
                                       'DeltaAlpha', 'DeltaBeta', 'DeltaGamma', 'DeltaTheta', 'ThetaAlpha', 'ThetaBeta', 'ThetaGamma', 'AlphaBeta', 'AlphaGamma', 'BetaGamma', 'BetaAlphaTheta', 'ThetaAlphaBeta', 'DeltaThetaAlphaBeta', 'DeltaThetaBetaGamma', 'BetaGammaDeltaTheta', ...
                                       });
    ...
    % Ensure the table is properly formatted and save it as a CSV file
    features = sortrows(features, 'MeanEEG'); % Example of sorting by MeanEEG
    writetable(features, outFile, 'Delimiter', ',', 'WriteRowNames', true); % Save as CSV with row names
end
