...
for i = 1:numel(files)
    ...
    if i == 1
        ...
        subplot(3,1,3);
        plot(t, ICA_eeg(1,:), "Color", "#9267FF");
        title('ICA EEG Data (Channel 1)');
        xlabel('Time (s)');
        ylabel('Amplitude');
        grid on;
        ...
        subplot(3,1,3);
        [pxx_ica, f_ica] = pwelch(ICA_eeg(1,:), hamming(1024), 512, 1024, Fs);
        plot(f_ica, 10*log10(pxx_ica), "Color", "#9267FF", "LineWidth", 1.3);
        grid on;
        xlabel('Frequency (Hz)');
        ylabel('PSD (dB/Hz)');
        title('ICA EEG PSD (Channel 1)');
