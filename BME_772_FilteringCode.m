...
function sig_bp = broad_bandpass(signal, Fs)
    ...
    [h, f] = freqz(bB, aB, 1024, 'half'); % Frequency response
    figure;
    subplot(2,1,1); % Create a subplot for magnitude response
    plot(f * (Fs / (2 * pi)), 20*log10(abs(h)), 'LineWidth', 1.5);
    grid on;
    title('Bandpass Filter Frequency Response - Magnitude');
    xlabel('Frequency (Hz)');
    ylabel('Magnitude (dB)');
    xlim([0 Fs/2]);
    ylim([-100 5]);
    ...
    plot(f * (Fs / (2 * pi)), unwrap(angle(h)), 'LineWidth', 1.5);
    grid on;
    title('Bandpass Filter Frequency Response - Phase');
    xlabel('Frequency (Hz)');
    ylabel('Phase (radians)');
    xlim([0 Fs/2]);
end
